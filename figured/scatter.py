...
from .common import get_name, any_names, preprocess_input, zip_cycle


def scatter(
		X=None, Y=None, Z=None, L=None,
		*,
		labels_from='y',
	...
	Parameters
	----------
	X, Y : array-like or Sequence[array-like]
		The x and y coordinates for each point.  Each should either be a
		single 1-d array, or a sequence of 1-d arrays.  If both are given as
		sequences, they must be aligned as each component array is paired in
		order.
	Z : array-like or Sequence[array-like], optional
		The marker size for each point.  Should either be a single 1-d array,
		or a sequence of 1-d arrays that aligns with similar sequences given in
		the `X`, `Y`, and `L` arguments.
	L : str or Sequence[str], optional
		The label for each array.  If given as a sequence of str it must align
		with similar sequences given in the `X`, `Y`, and `Z` arguments.

	Other Parameters
	----------------
	x_title, y_title: str, optional
		A label to apply to the {x,y} axis. If omitted and either of the `X`
		or `Y` arguments are given as named objects (e.g. pandas.Series), those
		names are used.
	axis_labels : bool, default True
		Set to False to suppress labelling the axes, even if labels can be
		inferred.
	...
	metadata : dict, optional
		A dictionary of meta-data to attach to the FigureWidget.
	sizemin : Number, optional
		The minimum marker size, can aid in legibility if some markers might
		be tiny.  Only used if `Z` is given.
	sizemode : {'diameter', 'area'}, default 'area'
		The rule used to scale markers.  Strongly recommended to leave this as 'area'.
	sizeref : Number, optional
		Set to scale the marker size. Only used if `Z` is given. Plotly recommends
		using the following formula to calculate a sizeref value:

		sizeref = 2. * max(array of size values) / (desired maximum marker size ** 2)

		Note that setting 'sizeref' to a value greater than 1, decreases the rendered
		marker sizes, while setting 'sizeref' to less than 1, increases the rendered
		marker sizes.
		See `https://plot.ly/python/reference/#scatter-marker-sizeref`_ for more
		information.
	...
	# When only X is given, assume it should actually be Y
	if Y is None:
		X, Y = Y, X

	# Preprocess to get correct formats
	X = preprocess_input(X)
	Y = preprocess_input(Y)
	Z = preprocess_input(Z)
	L = preprocess_input(L, to_type=str)

	# Get default axis labels
	if len(X)==1:
	...
	for x,y,z,label in zip_cycle(X,Y,Z,L):
	...
	return FigureWidget(data=traces, layout=layout, metadata=metadata)
